#Ejercicios Sakila Alejandro Blanco Villar
...
FROM staff s JOIN address a USING (address_id) JOIN city c USING (city_id) JOIN country co USING (country_id);
...
FROM customer cu JOIN address a USING (address_id) JOIN city c USING (city_id) JOIN country co USING (country_id);


#11. Direcciones de california que tengan ‘274’ en el número de teléfono
SELECT address
FROM address a JOIN city c USING (city_id)
WHERE a.district LIKE 'california' AND a.phone LIKE '%274%';


#12. Películas ‘Épicas’ (Epic) o ‘Brillantes’ (brilliant) que duren más de 180 minutos
SELECT title
FROM film f JOIN film_category fca USING (film_id) JOIN category ca USING (category_id)
WHERE f.length <= 180 AND ca.name LIKE 'Epic' OR ca.name LIKE 'brilliant';


#13. Películas que duren entre 100 y 120 minutos o entre 50 y 70 minutos
SELECT title, length
FROM film
WHERE length LIKE '10%' OR length LIKE '11%' OR length LIKE '5%' OR length LIKE '7%'
ORDER BY length;
...
WHERE (rental_rate = 0.99 OR rental_rate = 2.99) AND (rating = 'G' OR rating = 'R') AND (category.name LIKE '%Crocodile%');


#15 Direcciones de Ontario o de Punjab o que su código postal acabe en 5 o que su teléfono acabe en 5
SELECT *
FROM address
WHERE (city_id IN (SELECT city_id FROM city WHERE city = 'Ontario' OR city = 'Punjab')) OR (phone LIKE '%5') OR (postal_code LIKE '%5');


#16. Alquileres con un pago por encima de la media
SELECT title, amount
FROM film JOIN inventory USING (film_id) JOIN rental USING (inventory_id) JOIN payment USING (rental_id)
...
SELECT CONCAT(first_name, ' ', last_name) AS nombre
FROM actor
WHERE actor_id NOT IN (SELECT actor_id FROM film_actor JOIN film ON film.film_id = film_actor.film_id WHERE rating = 'R');


#18. Mostrar la categoría con menos películas
SELECT c.name categoria, COUNT(*) "Nº de peliculas"
FROM category c JOIN film_category fca ON c.category_id = fca.category_id
...
SELECT f.title titulo, COUNT(*) "Nº de peliculas"
...
SELECT COUNT(*) numero_peliculas, f.title titulo
FROM film f JOIN film_actor fa ON f.film_id = fa.film_id
GROUP BY f.film_id
HAVING COUNT(*) > 10;
...
SELECT film.title, COUNT(*) AS alquiler
FROM rental JOIN inventory ON rental.inventory_id = inventory.inventory_id JOIN film ON inventory.film_id = film.film_id
...
SELECT film.title, SUM(payment.amount) AS importe
FROM payment JOIN rental ON payment.rental_id = rental.rental_id JOIN inventory ON rental.inventory_id = inventory.inventory_id JOIN film ON inventory.film_id = film.film_id
...
SELECT CONCAT(a.first_name, ' ', a.last_name) AS nombre, COUNT(*) AS  Nº_pelis
FROM actor AS a JOIN film_actor AS fa ON a.actor_id = fa.actor_id
GROUP BY a.actor_id
ORDER BY Nº_pelis ASC
LIMIT 5;
...
SELECT CONCAT(first_name, ' ', last_name) AS nombre
FROM customer
WHERE customer_id NOT IN (SELECT customer_id FROM customer JOIN rental USING (customer_id) JOIN inventory USING (inventory_id) JOIN film USING (film_id) JOIN film_category USING (film_id) JOIN category USING (category_id) WHERE name = 'Documentary');